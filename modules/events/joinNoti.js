...
module.exports.run = async function({ api, event, Users }) {
	...
	if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
		api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "DC-Nam" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
		const fs = require("fs");
		return api.sendMessage("", event.threadID, () => api.sendMessage({body:`Thành Lê Hữu Đức\n[🇻🇳] Kết nối thành công\n[🇻🇳] Dấu lệnh / \n[🇻🇳] Admin : https://www.facebook.com/100056636595235`, attachment: fs.createReadStream(__dirname + "/cache/anhgai.jpg")} ,threadID));
	}
	else {
		try {

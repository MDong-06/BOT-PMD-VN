...
module.exports.handleEvent = function({ api, event }) {
  ...
  var tl = ["chào bạn tôi là bot của jay", "bạn gọi tôi có việc gì k ạ", "tôi yêu bạn lắm lắm bạn nhớ tương tác nha ", "Yêu cậu <3", "Hi, chào con ny của bot nha :3", "gọi admin có việc gì không?", "Sử dụng callad để liên lạc với admin!", "Em là bot cute nhất hành tinh", "Nói gì thế bé ", "jay đây~~~~", "Yêu admin nhất", "bot là cực cưng của của admin ", "Yêu thương admin nhất", "Gọi bot có việc gì không dạ ", "Sao thế  bae=)^^ ", "Đừng làm em đau ~~~~ hic :(", "Yêu bé ❤", "Tuyển phi công nè ạ", "Làm bồ em ko ạ? dui lắm", "Cậu cô đơn ko?", "Tuyển ghệ đút bự đi bay lắc", "Được của nó  :)))", "Em dthw như chủ của em ạ", "Đừng khen em ngại quá hí hí" ,"Làm ny admin ko ạ?", "Đừng spam em nha :<<", "biến đi sapm cái gì", "Bằng láu hủ tiếu kinh nghiêm đầy minh tuyển ghệ đít bự", "yncutes1💞", "Omg yêu mọi người ❤", "Spam gì đó tính yêu admin à ư ư ư ", "Yêu jay🙄 ko?", "Ny em đây rồi"];
  ...
  if ((event.body.toLowerCase() == "yn") || (event.body.toLowerCase() == "yn")) {
    return api.sendMessage("của ông chủ bot nha🎀", threadID);
  };

if ((event.body.toLowerCase() == "hí hí") ||  (event.body.toLowerCase() == "hihi")) {
    return api.sendMessage("nay bae của táo có việc gì vui dạ 🥰 kể bé ghe đi ", threadID);
  };
...
if ((event.body.toLowerCase() == "vg") ||  (event.body.toLowerCase() == "vâg")) {
  ...
  if ((event.body.toLowerCase() == ":))") || (event.body.toLowerCase() == "oi")) {
  ...
  if ((event.body.toLowerCase() == "hyen") || (event.body.toLowerCase() == "hyen")) {
    return api.sendMessage("của cậu chủ bot nhé", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "yn") || (event.body.toLowerCase() == "yn")) {
    return api.sendMessage("của ông chủ bot nha🎀",
threadID);
  };
  ...
  if ((event.body.toLowerCase() == "bot ơi") ||  (event.body.toLowerCase() == "bot ơi")) {
    return api.sendMessage("nói đi, ạ bot còn phải đi làm việc cho admin ạ nói nhanh đi ạ 🥲", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "chào tvm") || (event.body.toLowerCase() == "Chào tvm")) {
    return api.sendMessage("thành viên mới giới thiệu+show và set bd (nếu có)", threadID);
  };
if ((event.body.toLowerCase() == "Tui tvm") || (event.body.toLowerCase() == "tui tvm")) {
    return api.sendMessage("thành viên mới giới thiệu+show và set bd (nếu có)", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "adbot") || (event.body.toLowerCase() == "Adbot")) {
    return api.sendMessage("Jay", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "ad ") || (event.body.toLowerCase() == " ad")) {
    return api.sendMessage("admin rất đẹp trai :)))", threadID);
  };
 ...
 if ((event.body.toLowerCase() == "bsvv nha mn") || (event.body.toLowerCase() == "bsvv nha mn")) {
  ...
  if ((event.body.toLowerCase() == "Alo") || (event.body.toLowerCase() == "alo")) {
    return api.sendMessage("alo gì dạ? Yêu bot k nè hihihi", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "admin m là ai v bot") || (event.body.toLowerCase() == "admin m la ai v bot")) {
    return api.sendMessage("️Admin bot là ntt có gì dùng lệnh ad hoặc adm để biết thêm chi tiết", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "khoa") || (event.body.toLowerCase() == "Khoa")) {
    return api.sendMessage("️Kêu chủ của tôi gì á :)?", threadID);
  };

  if ((event.body.toLowerCase() == "Táo") || (event.body.toLowerCase() == "táo")) {
    return api.sendMessage("️Sao kêu ad có gì 0", threadID);
  };

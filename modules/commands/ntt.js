...
module.exports.handleReply = async function({ api, event, handleReply, getText }) {
    ...
    if (isNaN(body) && body.indexOf("c") == 0 || body.indexOf("cancel") == 0) {
        ...
        for (const singleIndex of index) {
            ...
            api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "𝑱𝒂𝒚-𝑩𝒐𝒕" : global.config.BOTNAME}`, handleReply.pending[singleIndex - 1].threadID, api.getCurrentUserID());
...
[🌐] 𝗙𝗯 𝗮𝗱𝗺𝗶𝗻: https://www.facebook.com/100075015857958`, attachment: fs.createReadStream(__dirname + "/cache/joinbox/join.mp4")} ,handleReply.pending[singleIndex - 1].threadID));
            count+=1;
...
module.exports.run = async function({ api, event, args, permission, handleReply }) {
    ...
    case "t":
    case "Thread": {
        const permission = ["100075015857958"];
    if (!permission.includes(event.senderID)) return api.sendMessage("Quyền lồn biên giới?", event.threadID, event.messageID);
     const { threadID, messageID } = event;
    const commandName = this.config.name;
    var msg = "", index = 1;
